...
interface HeaderSearchProps {
  ...
  links: {
    label: string;
    links: { label: string; onPress: (comp: string) => void }[];
  }[];
}
...
export function HeaderMenu({ links, liveCompetitions }: HeaderSearchProps) {
  ...
  const items = links.map((link) => {
    ...
    const liceCompetitionMenuItems = liveCompetitions?.map(
      (liveCompetition) => (
        <Menu.Item
          key={liveCompetition}
          component="a"
    ...
    if (regularCompetitionMenuItems) {
      return (
        ...
        >
          ...
          <Menu.Dropdown>
            {liceCompetitionMenuItems.concat(regularCompetitionMenuItems)}
          </Menu.Dropdown>
        </Menu>
      );
    }
        ...
        <div className={classes.inner}>
          <Group>
            <Link href={"/"}>
              <Image
                height={56 * 0.7}
                className={classes.backround}
                src={drakbonLoggaAndText.src}
